...
class SlideChannel(models.Model):
    ...
    custom_unblocked_third_party_domains = fields.Text(
        'User list of unblocked 3rd-party domains',
    ...
    def _compute_blocked_third_party_domains(self):
        for website in self:
            ...
            blocked_domain = full_list.split('\n')
            unblocked = website.custom_unblocked_third_party_domains.split('\n') if website.custom_unblocked_third_party_domains else False
            if unblocked:
                for unblock in unblocked:
                    blocked_domain.remove(unblock)
