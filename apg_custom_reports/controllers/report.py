...
class CommissionReportWizard(models.TransientModel):
				...
				def action_download_report(self):
								...
								headers=['Order Name','Customer Name','Affiliate ID(Sponsor)','Affiliate User ID(Sponsor)',
												'Affiliate Email(Sponsor)','Affiliate Plan','Commission','Phone','Email','Revenue','Product','Remarks',
												'Tag']
								...
								headers1=['From Date - To Date','Sponsor ID','Sponsor Name','Sponsor Email id','Commission','TDS','After TDS',
												'Account no.','Holder Name','IFSCCode','Status','Reason For Failed']
								for col,header in enumerate(headers1):
												worksheet1.write(0,col,header)
								
								# Fetch sale.order data based on date range
								sale_orders=self.env['sale.order'].search([('date_order','>=',self.from_date),('date_order','<=',self.to_date)])
																...
																if direct_partner:
																				if direct_partner.id not in partner_commission:
																								partner_commission[direct_partner.id]={
																												'partner':direct_partner,'commission_total':0.0}
																...
																if commission_partner:
																				if commission_partner.id not in partner_commission:
																								partner_commission[commission_partner.id]={
																												'partner':commission_partner,'commission_total':0.0}
																				partner_commission[commission_partner.id]['commission_total']+=line.partner_commission_amount or 0.0
								...
								attachment=self.env['ir.attachment'].create({
												'name':       'Commission_Report.xlsx','datas':base64.b64encode(output.read()),'type':'binary',
								...
								return {
												'type':'ir.actions.act_url','url':'/web/content/%s?download=true'%attachment.id,'target':'self',}
...
class RBLBankReportController(http.Controller):
				...
				def download_rbl_bank_report(self):
								...
								headers=['Payment Type','Cust Ref Number','Source Account Number','Source Narration',
												...
												'Destination Bank IFS Code','Beneficiary Name','Beneficiary Account Type','Email ']
								for col,header in enumerate(headers):
												worksheet.write(0,col,header)
...
class ICICIBankReportController(http.Controller):
				...
				def download_icici_bank_report(self):
								...
								headers=['PYMT_PROD_TYPE_CODE','PYMT_MODE','DEBIT_ACC_NO','BNF_NAME','BENE_ACC_NO','BENE_IFSC','AMOUNT',
												'DEBIT_NARR','CREDIT_NARR','MOBILE_NUM','EMAIL_ID','REMARK','PYMT_DATE','REF_NO','ADDL_INFO1','ADDL_INFO2',
												'ADDL_INFO3','ADDL_INFO4','ADDL_INFO5']
								for col,header in enumerate(headers):
...
class SalesCustomReportWizard(models.TransientModel):
				...
				def check_filters(self):
								...
								for rec in self:
												...
												if rec.joined_from and rec.joined_to:
																domain+=[('create_date','>=',rec.joined_from),('create_date','<=',rec.joined_to)]
												...
												if rec.from_date and rec.to_date:
																so_domain+=[('date_order','>=',rec.from_date),('date_order','<=',rec.to_date)]
												...
												# Fetch sale orders
												sale_orders=self.env['sale.order'].sudo().search(so_domain,order="name asc") if so_domain else self.env[
																'sale.order']
				...
				def action_download_report(self):
								...
								if sale_orders:
												for rec in sale_orders:
																if rec.amount_total>0:
																				...
																				products=", ".join(line.product_id.name for line in rec.order_line if line.product_id.bom_ids)
																				sponsor_name=(rec.order_line.filtered(lambda l:l.partner_commission_partner_id)[
								...
								attachment=self.env['ir.attachment'].create({
												'name':       'Sales Report.xls','datas':base64.b64encode(fp.getvalue()),'type':'binary',
												'store_fname':'sales_report.xls','mimetype':'application/vnd.ms-excel',})
...
class SalesCommissionReportWizard(models.TransientModel):
				...
				report_type=fields.Selection([('tot',"Total Counts Wise"),('par',"Partner Wise"),('price',"Price")],
								string="Report Type")
				
				mini=fields.Float("From Amount")
				maxi=fields.Float("To Amount")
				...
				def check_filters(self):
								...
								for rec in self:
												...
												if rec.from_date and rec.to_date:
																...
																domain+=[('order_id.date_order','>=',from_dt_utc.strftime(DEFAULT_SERVER_DATETIME_FORMAT)),
												...
												for line in order_lines:
																...
																if partner.id not in result:
																				result[partner.id]={
																								'partner_commission_partner_id':partner.name,'partner_commission_email':partner.email or '',
																								'partner_commission_phone':     partner.phone or '','partner_commission_amount':0.0,
																								'order_ids':                    []}
				...
				def action_download_report(self):
								...
								# Headers
								row=0
								if self.report_type=='tot':
												worksheet.write(row,0,'Sponsor Name',header_style)
												worksheet.write(row,1,'Total count ',header_style)
												worksheet.write(row,2,'Total Commission Amount',header_style)
								if self.report_type=='par':
												worksheet.write(row,0,'Sponsor Name',header_style)
								...
								if commissionData:
												for partner_id,data in commissionData.items():
																...
																if self.report_type=='par':
																				...
																				for order in orders:
																								commission_amt=sum(order.order_line.filtered(lambda l:l.partner_commission_partner_id).mapped(
																								...
																								row+=1
																if self.report_type=='tot':
																				print("ddddddddddddddddddddddddddddddd",data)
																				worksheet.write(row,0,data['partner_commission_partner_id'],sponsor_style)
																				worksheet.write(row,1,len(data['order_ids']),sponsor_style)
																				worksheet.write(row,2,data['partner_commission_amount'],sponsor_style)
																				row+=1
																
																if self.report_type=='price':
																				if data['partner_commission_amount']>=self.mini and data['partner_commission_amount']<=self.maxi:
																								worksheet.write(row,0,data['partner_commission_partner_id'],sponsor_style)
																								worksheet.write(row,2,data['partner_commission_email'],sponsor_style)
																								worksheet.write(row,3,data['partner_commission_phone'],sponsor_style)
																								worksheet.write(row,4,data['partner_commission_amount'],sponsor_style)
												...
												# # Get full SO records  # orders=self.env['sale.order'].browse(data['order_ids'])  #  # for order in orders:  # 				commission_amt=sum(order.order_line.filtered(lambda l:l.partner_commission_partner_id).mapped(  # 								'partner_commission_amount'))  # 				products=", ".join(order.order_line.mapped('product_id.display_name'))  # 				worksheet.write(row,1,order.partner_id.name or '',normal_style)  # 				worksheet.write(row,2,order.partner_id.email or '',normal_style)  # 				worksheet.write(row,3,order.partner_id.phone or '',normal_style)  # 				worksheet.write(row,4,commission_amt or '',normal_style)  # 				worksheet.write(row,5,order.amount_untaxed or 0.0,normal_style)  # 				worksheet.write(row,6,str(order.date_order.date()) if order.date_order else '',normal_style)  # 				worksheet.write(row,7,products or '',normal_style)  # 				row+=1
								...
								attachment=self.env['ir.attachment'].create({
												'name':       'Sales Commission Report.xls','datas':base64.b64encode(fp.getvalue()),'type':'binary',
												'store_fname':'sales_report.xls','mimetype':'application/vnd.ms-excel',})
