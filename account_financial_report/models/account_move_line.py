...
class AccountMoveLine(models.Model):
    ...
    def _compute_analytic_account_ids(self):
        """Compute analytic account IDs from analytic_distribution JSON field."""
        ...
        for record in self.filtered("analytic_distribution"):
            for key in record.analytic_distribution:
                # ✅ Only process numeric values to avoid ValueError
                valid_ids = [account_id for account_id in key.split(",") if account_id.isdigit()]
                for account_id in map(int, valid_ids):
                    batch_by_analytic_account[account_id] += record

        existing_account_ids = set(
            self.env["account.analytic.account"]
            .browse(batch_by_analytic_account.keys())  # ✅ FIX: Browse using keys, not dict
            .exists()
            .ids
        )

        # ✅ Clear previous analytic accounts before updating
        for record in self:
            record.analytic_account_ids = [Command.clear()]

        for account_id, records in batch_by_analytic_account.items():
            if account_id in existing_account_ids:
                records.write({"analytic_account_ids": [Command.link(account_id)]})
