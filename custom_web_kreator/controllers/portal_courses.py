...
class PortalMyCourses(http.Controller):
                ...
                if document and document in document_mapping:
                    ...
                    if document_front:
                        try:
                            ...
                            partner_values[doc_fields['front_file_field']] = base64.b64encode(front_data).decode(
                                'utf-8')
    ...
    def creator_landing_page(self, **post):
        ...
        approved_courses = request.env['slide.channel'].sudo().search(
            [('create_uid', '=', user.id), ('state', '=', 'course_preview')])
    ...
    @http.route('/creator/edit-landing-page', type='http', auth='public', website=True, csrf=True,
                methods=['GET', 'POST'])
    ...
    def live_session(self, **kwargs):
        user = request.env.user
        partner = user.partner_id  # Get related partner
        booking = request.env['lead.funnel'].sudo().search([('id', '=', kwargs.get('booking_id'))])
        if not booking:
            return http.request.redirect('/live/slot/booking')
        # {booking.slot_time.name}
        IST = tz.gettz('Asia/Kolkata')
        tzinfos = {'IST': IST}
        add_time = 0
        booking_slot = parser.parse(f'{booking.slot_date.strftime('%b %d %Y')} 3:40 PM IST', tzinfos=tzinfos)
        indian_timezone = pytz.timezone('Asia/Kolkata')
        if datetime.now(indian_timezone) < booking_slot:
            live_session = 'not started'
        elif datetime.now(indian_timezone)  > (booking_slot + timedelta(minutes=30)):
            live_session = 'ended'
            booking_slot = False
        else:
            live_session = 'started'
            add_time = (datetime.now(indian_timezone) - booking_slot).seconds
            booking.visited = True
            booking_slot = False

        # Check if the user is an Internal User or Creator
        return http.request.render('custom_web_kreator.live_session', {'live_session': live_session, 'time_slot': booking_slot.strftime('%b %d %Y %H:%M:%S') if booking_slot else '', 'time_add': '10', 'booking_id': booking.id, 'session_starts': booking.slot_time.name, 'user_name': booking.user_name})
    ...
    def live_slot_booking(self, **kwargs):
        ...
        if not booking_id:
            ...
            IST = tz.gettz('Asia/Kolkata')
            tzinfos = {'IST': IST}
            booking_slot = parser.parse(f'{booking_data.slot_date.strftime('%b %d %Y')} 6:49 PM IST', tzinfos=tzinfos)
            indian_timezone = pytz.timezone('Asia/Kolkata')
            if datetime.now(indian_timezone) > (booking_slot + timedelta(minutes=30)):
                request.session['booking_id'] = ''
                return http.request.redirect('/live/slot/booking')
    ...
    def live_slot_boo(self, **kwargs):
        ...
        lead = request.env['lead.funnel'].sudo().search(['|', ('email', '=', kwargs.get('email')), ('mobile', '=', kwargs.get('mobile')), ('expired', '=', False)])
        IST = tz.gettz('Asia/Kolkata')
        tzinfos = {'IST': IST}
        if lead:
            booking_slot = parser.parse(f'{lead.slot_date.strftime('%b %d %Y')} 6:49 PM IST', tzinfos=tzinfos)
            indian_timezone = pytz.timezone('Asia/Kolkata')
            if datetime.now(indian_timezone) > (booking_slot + timedelta(minutes=30)):
                lead.expired = True
                lead = False
        if not lead:
            ...
            lead = request.env['lead.funnel'].sudo().create({'user_name': kwargs.get('name'), 'email': kwargs.get('email'), 'name': sequence,
                                                      'mobile': kwargs.get('mobile'), 'slot_date': slot_date, 'slot_time': slot_time.id,
                                                             'describe': kwargs.get('describe'), 'city': kwargs.get('city'),
                                                             'income': kwargs.get('income'), 'affiliate_career': kwargs.get('affiliate_career')})
        request.session['booking_id'] = lead.id
        # Check if the user is an Internal User or Creator
        return http.request.redirect('/live/slot/booking')

    @http.route('/live/session/survey', type='http', auth='public', website=True)
    def live_session_survey(self, **kwargs):
        user = request.env.user
        partner = user.partner_id  # Get related partner
        booking = request.env['lead.funnel'].sudo().search([('id', '=', kwargs.get('lead'))])
        survey_id = request.env['live.session.survey'].sudo().create({'name': booking.id, 'rating': kwargs.get('helpful'),
                                                          'delivery': kwargs.get('delivery'), 'friend': kwargs.get('friend'),
                                                          'platform': kwargs.get('platform'), 'starting': kwargs.get('staring')})
        booking.survey_id = survey_id.id
        booking.finished = True
        return http.request.render('custom_web_kreator.live_session_survey')
            ...
            try:
                ...
                if kwargs.get('type') == 'zerotoo':
                    if int(ved_no) == 1:
                        ...
                        <iframe width="100%" height="60%" src="https://www.youtube.com/embed/zxgCpvxANA8?si=lhk9_CvGSq7PURs_" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                        </div>
                    ...
                    if int(ved_no) == 2:
                        video_data = Markup('''
                        <div style="width: 80dvw; height: 100dvh;" class="d-none d-md-block">
                        <iframe width="100%" height="60%" src="https://www.youtube.com/embed/hvAFeAXHkQw?si=2x-3qIo8-GVNSIue" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                        </div>
                        <iframe width="100%" height="auto" class="d-block d-md-none" src="https://www.youtube.com/embed/hvAFeAXHkQw?si=2x-3qIo8-GVNSIue" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                        ''')
                        next_vid = Markup('''
                        <a onclick="openVideo(3, 'zerotoo')" class="border1 rounded p-2" style="background-color:#ffc107; cursor:pointer;">
                                            Next
                                            <i class="ri-arrow-right-line"></i>
                                        </a>
                        ''')
                    if int(ved_no) == 3:
                        video_data = Markup('''
                        <div style="width: 80dvw; height: 100dvh;" class="d-none d-md-block">
                        <iframe width="100%" height="60%" src="https://www.youtube.com/embed/S6wJmx4nhrs?si=Qbk-mRKBq7i9Gn2L" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                        </div>
